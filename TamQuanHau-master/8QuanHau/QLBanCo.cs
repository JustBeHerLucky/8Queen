...
namespace _8QuanHau
{
    public class QLBanCo
    {
        ...
        private Image hinhconco3 = Image.FromFile(Application.StartupPath + "\\resources\\Hau3.png");
        ...
        private void Btn_Click(object sender, EventArgs e)
        {
            ...
            if (btn.BackgroundImage == Player[CurrentPlayer].Mark || btn.BackgroundImage == hinhconco3)
            {
            ...
            if (playerMarked != null)
                ...
                else
                {
                        ...
                        for (int row =0; row <8; row++)
                    {
                        ...
                        {
                            if (Matrix[row][col].BackgroundImage == Player[CurrentPlayer].Mark)
                            {    
                                if (!CheckNonHave(row, col))
                                {
                                    MessageBox.Show("Thua rồi, kiểm tra lại đi!");
                                    return;
                                }
                            }
        ...
        bool CheckNonHave (int row, int col)
        {
            for (int i=0; i<8; i++)
            {
                if (Matrix[row][i].BackgroundImage == Player[CurrentPlayer].Mark && i != col) return false;
            }
            for (int i = 0; i < 8; i++)
            {
                if (Matrix[i][col].BackgroundImage == Player[CurrentPlayer].Mark && i != row) return false;
            }
            for (int i=0; i<8; i++)
            {
                for(int j=0; j<8; j++)
                {
                    if(Matrix[i][j].BackgroundImage == Player[CurrentPlayer].Mark && i!= row && j!= col)
                    {
        ...
        public void Checkkk()
        {
            for (int row = 0; row < 8; row++)
            {
                for (int col = 0; col < 8; col++)
                {
                    if (Matrix[row][col].BackgroundImage == Player[CurrentPlayer].Mark && !CheckNonHave(row, col))
                    {
                        Matrix[row][col].BackgroundImage = hinhconco3;
                        return;
